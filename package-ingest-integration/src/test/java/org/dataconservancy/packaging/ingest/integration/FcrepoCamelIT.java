...
public class FcrepoCamelIT extends CamelTestSupport {

	private static final String HEADER_FCREPO_BASEURI = "test.fcrepo.baseURI";
	private static final String HEADER_FCREPO_CANONICAL_LOCATION = "test.fcrepo.canonical.location";
	private static final String HEADER_FCTRPO_TX_BASEURI = "test.fcrepo.txBaseURI";
	...
	public void putObjectFcrepoCamelTest() throws Exception {
				...
				new HashMap<String, Object>() {
					{
						...
						put(Exchange.CONTENT_TYPE, "text/turtle");
					}
				});
		...
		assertEquals(HttpStatus.SC_CREATED,
				resultExchange.getIn().getHeader(Exchange.HTTP_RESPONSE_CODE));
	...
	public void putObjectHttpTest() throws Exception {
		...
		template.sendBodyAndHeaders("direct:httpComponentRequest",
				...
				new HashMap<String, Object>() {
					...
					{
						put(HEADER_FCREPO_BASEURI, FEDORA_BASEURI);
						put(Exchange.HTTP_URI, uriOfContainer);
						put(Exchange.HTTP_METHOD, "POST");
						put(Exchange.CONTENT_TYPE, "text/turtle");
					}
				});
		...
		String newObjectURI = responseMessage.getHeader(
				HEADER_FCREPO_CANONICAL_LOCATION, String.class);
	...
	protected RouteBuilder createRouteBuilder() {
		...
		return new RouteBuilder() {
			...
			public void configure() throws Exception {
				 ...
				 * endpoint
				 */
				from("direct:camelComponentRequest").to(fcrepoEndpoint).to(
						"mock:response");
				 ...
				 */
				from("direct:startTransaction").enrich(
						"direct:_doStartTransaction",
				...
				from("direct:_doStartTransaction")
						...
						.setHeader(
								Exchange.HTTP_URI,
								simple(String.format("${in.header.%s}/fcr:tx",
				 ...
				 */
				from("direct:commitTransaction").enrich(
						"direct:_doCommitTransaction", ((orig, resp) -> orig));
				...
				from("direct:canonicalize")
						.process(
								exchange -> exchange
										.getIn()
										.setHeader(
												HEADER_FCREPO_CANONICAL_LOCATION,
												exchange.getIn()
														.getHeader("Location",
																String.class)
														.replaceFirst(
				 ...
				 */
				from("direct:canonicalizeViaLink").enrich(
						"direct:_doHead",
						((orig, resp) -> {
							orig.getIn()
									.setHeader(
