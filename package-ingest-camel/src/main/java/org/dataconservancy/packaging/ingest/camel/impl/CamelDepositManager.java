...
public class CamelDepositManager implements DepositManager {
    ...
    private final Map<DepositWorkflow, CamelContext> contexts = new ConcurrentHashMap<>();

    private final Queue<WorkflowConfiguration> pendingWorkflows = new ConcurrentLinkedQueue<>();
    ...
    public void initDepositWorkflow(WorkflowConfiguration wf) {
        if (active.get()) {
            ...
            try {
                ...
                final CamelContext context = cxtFactory.newContext("", registry);

                final Properties p = copy(globalProperties);
    ...
    public void removeDepositWorkflow(DepositWorkflow workflow) {
        ...
        final CamelContext cxt = contexts.remove(workflow);
    ...
    static void updateProperties(Properties props, Map<String, Object> map) {
        props.putAll(map.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> entry.getValue()
                .toString())));
    }
